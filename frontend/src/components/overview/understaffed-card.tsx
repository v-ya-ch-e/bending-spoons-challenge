...
type UnderstaffedCardProps = {
  projects: Project[]
  selectedProjectId?: number
  onProjectOpen: (projectId: number) => void
}

export function UnderstaffedCard({
  projects,
  selectedProjectId,
  onProjectOpen,
}: UnderstaffedCardProps) {
  ...
  return (
    <RailCard>
      <RailCardHeader title="Understaffed" count={understaffed.length} />
      {understaffed.length === 0 ? (
        <RailEmptyState label="All projects fully staffed" />
      ) : (
        <ScrollArea className="-mx-2 min-h-0 flex-1">
          <ul className="flex flex-col px-2">
            {understaffed.map(({ project, gap }) => {
              ...
              return (
                <li key={project.id}>
                  ...
                  >
                    ...
                    <span className="w-7 shrink-0 text-right text-sm font-semibold text-[#EF4444] tabular-nums">
                      −{gap}
                    </span>
                  </button>
                </li>
