...
- `project_documentation.project_id` uses `ON DELETE CASCADE`, so deleting a project deletes its generated documentation row.

## Project Documentation

Database table: `project_documentation`

Stored fields:

- `id`: integer primary key, auto-incremented.
- `project_id`: required project ID, unique. Each project has at most one documentation row.
- `status`: required enum, one of `pending`, `running`, `ready`, `failed`.
- `content_markdown`: markdown documentation body. Empty while queued or failed.
- `source_repositories`: JSON array of repository URLs used for generation.
- `source_snapshot`: nullable JSON object with GitHub context metadata collected during generation.
- `model_metadata`: nullable JSON object with LLM model and prompt metadata.
- `last_error`: nullable text for the latest generation failure.
- `last_generated_at`: nullable datetime set when generated content is successfully written.
- `created_at`: datetime generated by MySQL.
- `updated_at`: datetime updated by MySQL.
...
```json
{
  ...
  "status": "running",
  "content_markdown": "",
  "source_repositories": ["https://github.com/mixing-spooners/evernote-core"],
  "source_snapshot": { "repositories": [] },
  "model_metadata": { "model": "gpt-4o", "prompt_version": "project_documentation_v1" },
  "last_error": null,
  "last_generated_at": null
}
```

Update payloads are partial and may include any stored field except `id`, `project_id`, `project_name`, `created_at`, or `updated_at`.

`PUT /projects/{project_id}/documentation` upserts the project-scoped row. It returns `201` when creating a new row and `200` when updating an existing row.
...
```json
{
  ...
  "project_name": "Evernote",
  "status": "ready",
  "content_markdown": "# Evernote\n\nGenerated documentation...",
  "source_repositories": ["https://github.com/mixing-spooners/evernote-core"],
  "source_snapshot": { "repositories": [{ "name": "evernote-core" }] },
  "model_metadata": { "model": "gpt-4o", "prompt_version": "project_documentation_v1" },
  "last_error": null,
  "last_generated_at": "2026-04-26T00:00:00",
