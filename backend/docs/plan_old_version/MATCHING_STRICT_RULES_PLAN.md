...
Step 1 returns candidate plans ordered by deterministic strict score.
...
- The target employee.
- Projects below target headcount.
- Projects whose required skills match the target employee.
- The employee's current projects, if moving or reducing assignments is allowed.
- Projects listed in employee preferences.

Exclude by default:

- Projects that are already fully staffed and gain no skill coverage.
- Projects that would create too many concurrent assignments for the employee.

This use case should not evaluate unrelated employee moves unless the target
employee's current project needs a direct replacement.
...
- Employee-scoped runs only evaluate the target employee unless replacement is
  required.
...
- Employee lifecycle status: active, onboarding, transitioning, unavailable,
  leaving, inactive.
