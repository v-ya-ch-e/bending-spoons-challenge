...
- A run with strict-rule candidates must call the LLM evaluator, persist ranked `matching_recommendations`, set `selected_candidate_plan_id`, and return `recommendations` in the API response.
